...
export default function Index() {
    ...
    const AddTodo = async () => {
        const y = await axios.post("http://localhost:5000/todos/add-todo", { data1, Emoji })
        getData()
    }
    const getData = async () => {
        ...
        settodoData(data.result)
    }
    const deleteTask = async (item) => {
        const result = await axios.delete(`http://localhost:5000/todos/destroy/${item._id}`)
        getData();

    }
    const updateTask = async (item) => {
        const result = await axios.put(`http://localhost:5000/Todos/${item.id}`, { item, Emoji })
        getData()
